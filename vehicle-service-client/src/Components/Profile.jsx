...
const Profile = () => {
  const email ='ffggff@example.com';
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        // Fetch user information by email using POST request
        ...
        // Fetch reservations by email using GET request
        const reservationsResponse = await axios.post(
          `http://localhost:5000/api/reservations/reservationinfo`,{ email }
        );
